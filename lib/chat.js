...
Chat = {
	...
	initialize : function (server, sessionStore) {
		this.everyone = nowjs.initialize(server, {socketio: {transports: ['xhr-polling', 'jsonp-polling']}});
	...
	loadExistingMessages : function () {
		...
		Room.findOne({ name: that.now.room }, function (err, room) {
			Message.find({ roomid: room.id }, function(err, messages) {
				var client_user = {
					id : that.user.clientId,
					name : that.user.session.username,
					avatar : '',
					email : that.user.session.email,
					isserver : false
				}
				for (var message in messages) {
					var dt = messages[message].created;
					var client_message = {
				...
				that.now.loadRoomMessages(client_user, existing_messages, false);
			});			
		});
	},
