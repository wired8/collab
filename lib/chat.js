...
Chat = {
    ...
    this.everyone.now.userIsActive = function (room) {
    	  console.log('nowjs.getGroup(room): ' + room);
		...
		this.getPersistantRooms(page, limit, this, function () {
			nowjs.getGroups(function (groups) {
				...
				for (var i = startIndex; i < endIndex; i++) {
					...
					room.messagecount = 0;
					room.id = grp.id;
					room.private = grp.private;
					room.isadmin = true;
					room.enabled = grp.enabled;
					rooms.push(room);
	...
	getPersistantRooms : function (page, limit, context, callback) {
		var that = this;
		console.log('getPersistantRooms');
		// Load persistant rooms
		Room.find({enabled: 1}, function (err, rooms) {
			for (var r in rooms) {
				var group = nowjs.getGroup(rooms[r].url);
				group.description = rooms[r].description;
				group.id = rooms[r].id;
				group.url = rooms[r].url;
				group.private = rooms[r].private;
				group.enabled = rooms[r].enabled;
			}
			callback();
	...
	setupGroup : function (group) {
		...
		if (!group.hasOwnProperty('_events')) {
			group.on('join', function () {
				 ...
				 */
				that.now.roomurl = group.groupName;
			...
			group.on('leave', function (data) {
				var currentTime = new Date();
				var hours = currentTime.getHours();
				var minutes = currentTime.getMinutes();
				var self = this;

				that.sendMessage(self.user.session.username + ' left', this, true);
				group.now.removeUser(self.user.session.user_id);
